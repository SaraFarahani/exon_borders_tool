...
my $gene_id = 'ENSG001';
my $transcript_id = 'ENST001';
my $exon_id = 'ENSE001';
my $start = 1;
my $end = 10;
my $obj = Bio::Seq->new(-display_id => "$gene_id", -transcript_id => "$transcript_id", -exon_id => "$exon_id");
my $out = Bio::SeqIO->new(-file=> ">bioseq.txt", -format=>'fasta');
$out->write_seq($obj);
